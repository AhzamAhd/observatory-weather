...
def load_atmospheric():
    ...
    for _, row in df.iterrows():
        atm = get_full_atmospheric_analysis({
            ...
            "temp_700hpa":      row.get("temp_700hpa"),
            "temp_600hpa":      row.get("temp_600hpa"),
            "temp_500hpa":      row.get("temp_500hpa"),
            "geopot_850hpa":    row.get("geopot_850hpa"),
            "geopot_700hpa":    row.get("geopot_700hpa"),
            "geopot_600hpa":    row.get("geopot_600hpa"),
            "geopot_500hpa":    row.get("geopot_500hpa"),
            "wind_850hpa":      row.get("wind_850hpa"),
            "wind_700hpa":      row.get("wind_700hpa"),
            "wind_600hpa":      row.get("wind_600hpa"),
            "wind_500hpa":      row.get("wind_500hpa"),
...
def load_data():
    ...
    def _quality_row(r):
        ...
        profile = {
            ...
            "temp_700hpa":   r.get("temp_700hpa"),
            "temp_600hpa":   r.get("temp_600hpa"),
            "temp_500hpa":   r.get("temp_500hpa"),
            "geopot_850hpa": r.get("geopot_850hpa"),
            "geopot_700hpa": r.get("geopot_700hpa"),
            "geopot_600hpa": r.get("geopot_600hpa"),
            "geopot_500hpa": r.get("geopot_500hpa"),
            "wind_850hpa":   r.get("wind_850hpa"),
            "wind_700hpa":   r.get("wind_700hpa"),
            "wind_600hpa":   r.get("wind_600hpa"),
            "wind_500hpa":   r.get("wind_500hpa"),
...
""", unsafe_allow_html=True)
    ...
    _features = [
        ...
        ("📝", "#e74c3c", "Plan an Observation", "Build a real observing proposal: targets, time, moon, SNR-solved exposures, and a best-months calendar."),
...
# ═══════════════════════════════════════════════════════
# Plan an Observation — guided wizard (replaces Proposal Planner)
# ═══════════════════════════════════════════════════════
if selected_page == "Plan an Observation":
    page_header("🗺️", "Plan an Observation",
        "A step-by-step guide from target to a finished observing plan — "
        "visibility, best window, conditions, exposure time and expected "
        "signal-to-noise, all in one place.")

    from planning_wizard import render_planning_wizard
    render_planning_wizard(df, utcnow, load_atmospheric_cached)
...
if selected_page == "SNR Calculator":
    ...
    _sig_m = result.get("sigma_mag")
    m1.metric("SNR", snr_val,
              delta=(f"±{_sig_m} mag" if _sig_m else None),
              delta_color="off",
              help="SNR of the measurement. The ± value is the resulting "
                   "photometric uncertainty on the magnitude, "
                   "σ_m = 1.0857 / SNR.")
