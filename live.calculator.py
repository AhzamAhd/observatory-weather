...
from observing_window import get_observing_window
...
def calculate_live_conditions(obs_row):
    ...
    # ── Observing window ──────────────────────────────────
    win = get_observing_window(
        lat, lon,
        obs_row["observatory"],
        obs_row.get("country", "Unknown"),
        w_score
    )
    ...
    return {
        ...
        "country":          obs_row.get("country", "Unknown"),
        "latitude":         lat,
        "longitude":        lon,
        "altitude_m":       alt,
        ...
        "peak_score":       peak["peak_score"] if peak else 0,
        "total_good_hours": peak["total_good_hours"] if peak else 0,
        "hourly_data":      peak["hourly_data"] if peak else [],
        # Timestamp
